...
const {systemCategoryId, systemPermission} = require("./sharWarehouseConstants");
const requirePermissions = require("../../middlware/requirePermissions");
router.post("/shar-warehouse/transfer", requirePermissions(systemPermission),
    body("products.*.fromProductStorage").exists(),
    body("products.*.product.barcode").isString().custom(async (barcode, {req}) => {
        const product = await Product.findByPk(barcode);
        if (!product || product.categoryId === systemCategoryId) {
            throw new Error("ئەم بەرهەمە بوونی نییە");
        }
    }),
            ...
            await sequelize.transaction(async (t) => {
                ...
                for (const p of products) {
                    ...
                    const productStorage = await ProductStorage.findOne({
                        where: {
                            productStorageId: fromProductStorage.productStorageId,
...
router.delete('/shar-warehouse/transfer/:transferId', requirePermissions(systemPermission), async (req, res) => {
    const {transferId} = req.params;
    try {
        await sequelize.transaction(async (t) => {
            const transfer = await Transfer.findByPk(transferId, {transaction: t});
            if (!transfer || transfer.systemCategoryId !== systemCategoryId) {
...
router.get("/shar-warehouse/transfers", requirePermissions(systemPermission), async (req, res) => {
    try {
        const transfers = await Transfer.findAll({
            where: {systemCategoryId},
            include: [{model: Storage, as: "toStorage"}]
        });
