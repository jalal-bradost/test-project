...
router.get(
  ...
  [
    query("departmentType")
      ...
      .isIn(["ALL", "OP", "ICU", "SW"]) // Added "ALL" as a valid option
      .withMessage("Invalid departmentType. Must be ALL, OP, ICU, or SW."),
    // productTypeFilter and date filters removed from backend validation here,
    // as per the frontend separation and 'disposable only' requirement.
  ],
  returnInCaseOfInvalidation, // Middleware to handle validation errors
  // requirePermissions(permissionMap.VIEW_REPORTS), // Example: Uncomment and adjust permissions
  async (req, res) => {
    try {
      // The frontend for patient-products handles date and product type filtering client-side
      // as per our previous discussion. The backend's role here is to fetch the raw data.
      const { departmentType } = req.query; // Only departmentType from query

      const allPatientsWithProducts = []; // Stores the final structured data
      const allBarcodesAcrossAllPatients = new Set(); // Collects all barcodes for bulk fetching

      // --- Fetch and process OP Data ---
      if (!departmentType || departmentType === "ALL" || departmentType === "OP") {
        const opDatas = await OPData.findAll({
          include: [
            { model: Patient, attributes: ['patientId', 'fullname', 'birthdate'] },
            { model: OPOperationType, attributes: ['name'] },
          ],
        });

        opDatas.forEach(opData => {
          ...
          // Helper function to create product entry for a given sub-department
          const addProductEntry = (items, subDepartmentName) => {
            if (items && items.length > 0) {
              const productsForSubDepartment = [];
              items.forEach(item => {
                if (item && item.barcode && item.product && item.quantity !== undefined) {
                  productsForSubDepartment.push({
                    barcode: item.barcode,
                    product: {
                      id: item.product.code || item.product.id,
                      ...
                      isProductInPharmacyStorage: false, // Will be updated below
                      productCost: 0, // Will be updated below
                      createdAt: item.createdAt || "", // Added createdAt to the product object
                    },
                    quantity: item.quantity,
                    createdAt: item.createdAt || "", // Keep createdAt for the usage record
                  });
                  allBarcodesAcrossAllPatients.add(item.barcode);
                }
              });

              if (productsForSubDepartment.length > 0) {
                allPatientsWithProducts.push({
                  recordId: opData.opId,
                  recordType: 'OP',
                  patient: patientInfo,
                  department: subDepartmentName, // Set department to specific sub-department
                  departmentType: 'OP',
                  products: productsForSubDepartment,
                });
              }
            }
          };

          addProductEntry(opData.perfusionItems, 'PERFUSION');
          addProductEntry(opData.anesthesiaItems, 'ANESTHESIA');
          addProductEntry(opData.scrubNurseItems, 'SCRUB NURSE');
        });
      }
      ...
      if (!departmentType || departmentType === "ALL" || departmentType === "ICU") {
        const icuDatas = await ICUData.findAll({
          include: [
            { model: Patient, attributes: ['patientId', 'fullname', 'birthdate'] },
            { model: ICUOperationType, attributes: ['name'] },
          ],
        });

        icuDatas.forEach(icuData => {
          // Record-level department name for ICU
          const recordDepartmentName = icuData.ICUOperationType ? icuData.ICUOperationType.name : 'ICU';
          const patientProductsForRecord = [];
          if (icuData.items) {
            icuData.items.forEach(item => {
              if (item && item.barcode && item.product && item.quantity !== undefined) {
                patientProductsForRecord.push({
                  barcode: item.barcode,
                  product: {
                    id: item.product.code || item.product.id,
                    name: item.product.name,
          ...
          if (patientProductsForRecord.length > 0) {
            allPatientsWithProducts.push({
              recordId: icuData.icuId,
              recordType: 'ICU',
              patient: icuData.Patient ? {
                id: icuData.Patient.patientId,
                fullname: icuData.Patient.fullname,
                birthdate: icuData.Patient.birthdate,
              } : null,
              department: recordDepartmentName, // Record-level department name
              departmentType: 'ICU',
              products: patientProductsForRecord,
            });
          }
        });
      }
      ...
      if (!departmentType || departmentType === "ALL" || departmentType === "SW") {
        const swDatas = await SWData.findAll({
          include: [
            { model: Patient, attributes: ['patientId', 'fullname', 'birthdate'] },
            { model: SWOperationType, attributes: ['name'] },
          ],
        });

        swDatas.forEach(swData => {
          // Record-level department name for SW
          const recordDepartmentName = swData.SWOperationType ? swData.SWOperationType.name : 'SURGICAL WARD';
          const patientProductsForRecord = [];
          if (swData.items) {
            swData.items.forEach(item => {
              if (item && item.barcode && item.product && item.quantity !== undefined) {
                patientProductsForRecord.push({
                  barcode: item.barcode,
                  product: {
      ...
      // Perform a single bulk fetch for all product-related info across all patients
      const bulkProductInfo = await getBulkProductInfo(Array.from(allBarcodesAcrossAllPatients));

      let flattenedData = [];
      allPatientsWithProducts.forEach(record => {
        record.products.forEach(productItem => {
          const productInfoFromBulk = bulkProductInfo[productItem.barcode];
          
          // Populate productCost and isProductInPharmacyStorage
          productItem.product.productCost = productInfoFromBulk ? calculateProductCost(productItem.product, productInfoFromBulk.latestInvoicePrice) : 0;
          productItem.product.isProductInPharmacyStorage = productInfoFromBulk ? productInfoFromBulk.isProductInPharmacyStorage : false;

          // Flatten the structure for easier filtering and display
          flattenedData.push({
            ...
            productSpecialPriceUSD: productItem.product?.specialPriceUSD,
            productPerBox: productItem.product?.perBox,
            isProductInPharmacyStorage: productItem.product?.isProductInPharmacyStorage,
            productCost: productItem.product?.productCost,
            sourceCategory: productItem.product?.sourceCategory, // Only for OP items
            quantity: productItem.quantity,
            itemCreatedAt: productItem.createdAt // This is the createdAt for the specific item usage
          });
        });
      });
      ...
      flattenedData = flattenedData.filter(item => {
        return item.isProductInPharmacyStorage === false;
      });

      // Removed productTypeFilter logic from here, as all are now disposable

      // Sort results by recordId descending for consistency (optional)
      flattenedData.sort((a, b) => {
        if (a.recordId && b.recordId) return b.recordId - a.recordId;
        return 0;
      });
      ...
      return res.status(500).json({ message: "An error occurred on the server while fetching patient products." });
    }
  }
);


module.exports = router; // Export the router containing the new report routes