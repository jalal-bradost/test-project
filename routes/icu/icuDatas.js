...
router.post("/icu/data", [
    ...
    try {
        ...
        for (const icuOperationTypeId of req.body.icuOperationTypes) {
            await ICUDataOperationTypeJunction.create({icuId: icuData.icuId, icuOperationTypeId}, {transaction: t});
        }
        await t.commit();
...
router.get("/icu/data/:icuId", param("icuId").isInt(), returnInCaseOfInvalidation, async (req, res) => {
    try {
...
router.put("/icu/data/:icuId/details", [
    param("icuId").isInt().custom(async icuId => {
        const icuData = await ICUData.findByPk(icuId);
        if (!icuData) {
    ...
    try {
        const {icuId} = req.params;
        const {staffs, entryTime, exitTime, doctorId, icuOperationTypes} = req.body;
        await ICUData.update({staffs, entryTime, exitTime, doctorId}, {where: {icuId}, transaction});
        if (icuOperationTypes) {
            await ICUDataOperationTypeJunction.destroy({where: {icuId}, transaction});
            for (const icuOperationTypeId of icuOperationTypes) {
                await ICUDataOperationTypeJunction.create({icuId, icuOperationTypeId}, {transaction});
            }
        }
...
// Delete
router.delete("/icu/data/:icuId", param("icuId").isInt(), returnInCaseOfInvalidation, async (req, res) => {
    try {
        const icuData = await ICUData.findByPk(req.params.icuId);
        if (!icuData) {
            return res.status(400).json({message: "بوونی نییە"});
...
router.post("/icu/item-refund", requirePermissions([permissionMap.accountant]),
    body("icuId").isInt(),
    body("quantity").isInt(),
        ...
        try {
            const result = await sequelize.transaction(async (t) => {
                ...
                if (quantity > selectedProduct.quantity) {
                    ...
                    const item = icuData.items.find(item => item.barcode === selectedProduct.barcode && item.quantity === selectedProduct.quantity)
...
router.post("/icu/merge", requirePermissions([permissionMap.accountant]),
    body("icuCases").isArray({min: 2}),
    body("icuCases.*.icuId").isInt().custom(async icuId => {
        const icuData = await ICUData.findByPk(icuId);
        if (!icuData) {
            throw new Error("ICU data not found");
        }
    ...
    returnInCaseOfInvalidation, async (req, res) => {
        ...
        try {
            const result = await sequelize.transaction(async (t) => {
                ...
                const totalPrice = icuDatas.reduce((acc, current) => acc + current.totalPrice, 0)
                const items = icuDatas.reduce((acc, current) => [...acc, ...current.items], [])
                ...
                const uniqueOperationTypes = new Set(icuDatas.map(icu => icu.ICUOperationTypes.map(icuOperationType => icuOperationType.icuOperationTypeId)).flat())
                await ICUDataOperationTypeJunction.bulkCreate(Array.from(uniqueOperationTypes).map(opTypeId => ({
                    icuId: mergedIcu.icuId,
                    icuOperationTypeId: opTypeId
                })), {transaction: t})
                await ICUData.destroy({where: {icuId: icuCases.map(icu => icu.icuId)}, transaction: t})
