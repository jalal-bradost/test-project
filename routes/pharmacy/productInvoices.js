...
router.post("/pharmacy/product-invoice", requirePermissions(systemPermission), body("barcode").notEmpty().custom(async barcode => {
...
router.put("/pharmacy/product-invoice", requirePermissions(systemPermission), body("productInvoiceId").notEmpty().custom(async productInvoiceId => {
...
router.delete("/pharmacy/product-invoice/:productInvoiceId", requirePermissions(systemPermission), param("productInvoiceId").isInt(), returnInCaseOfInvalidation, async (req, res) => {
    const {productInvoiceId} = req.params
    try {
        const invoice = await ProductInvoice.findByPk(productInvoiceId, {include: [{model: Product}]})
        if (!invoice) {
            return res.status(400).json({message: "ئەو بەشە بوونی نییە"})
        }
        if (invoice.Product.systemCategoryId !== systemCategoryId) {
            return res.status(400).json({message: "ئەو بەشە بوونی نییە"})
        }
