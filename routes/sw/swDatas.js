  ...
  async (req, res) => {
    try {
      const swData = await SWData.findByPk(req.params.swId, {
        include: [{ model: Patient }, { model: SWOperationType }],
      });
      if (!swData) {
  ...
  async (req, res) => {
    ...
    try {
      const result = await sequelize.transaction(async (t) => {
        ...
        const totalPrice = swDatas.reduce(
          (acc, current) => acc + current.totalPrice,
          0
        );
        const items = swDatas.reduce(
        ...
        const uniqueOperationTypes = new Set(
          swDatas
            .map((sw) =>
              sw.SWOperationTypes.map(
                (swOperationType) => swOperationType.swOperationTypeId
              )
            )
            .flat()
        );
        await SWDataOperationTypeJunction.bulkCreate(
