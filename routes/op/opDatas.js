  ...
  async (req, res) => {
    ...
    try {
      ...
      for (const opOperationTypeId of req.body.opOperationTypes) {
        await OPDataOperationTypeJunction.create(
          { opId: opData.opId, opOperationTypeId },
          { transaction: t }
        );
      }
      await t.commit();
...
router.get("/op/data", async (req, res) => {
  try {
    ...
    const filteredOpData = await Promise.all(
      opDatas.map(async (model) => {
        const data = model.get({ plain: true });
        return {
          ...data,
          perfusionItems: await cleanItems(data.perfusionItems),
          anesthesiaItems: await cleanItems(data.anesthesiaItems),
          scrubNurseItems: await cleanItems(data.scrubNurseItems),
        };
      })
    );
  ...
  async (req, res) => {
    ...
    try {
      ...
      opData.scrubNurseItems = [
        ...opData.scrubNurseItems,
        ...req.body.scrubNurseItems,
      ];
      opData.perfusionItems = [
        ...opData.perfusionItems,
        ...req.body.perfusionItems,
      ];
      opData.anesthesiaItems = [
        ...opData.anesthesiaItems,
        ...req.body.anesthesiaItems,
      ];
  ...
  async (req, res) => {
    ...
    try {
      ...
      await OPData.update(
        ...
        { where: { opId }, transaction }
      );
      if (opOperationTypes) {
        await OPDataOperationTypeJunction.destroy({
          where: { opId },
          transaction,
        });
        for (const opOperationTypeId of opOperationTypes) {
          await OPDataOperationTypeJunction.create(
            { opId, opOperationTypeId },
            { transaction }
          );
        }
      }
...
const adjustProductStorage = async (items, storageId, t) => {
  for (const item of items) {
    ...
    if (quantityToDeduct > 0) {
      throw new Error(
        `Not enough quantity in storage for barcode ${item.barcode} and storageId ${storageId} with quantity of ${item.quantity}`
      );
    }
  }
};
...
router.post(
  "/op/anesthesia-item-refund",
  requirePermissions([permissionMap.accountant, permissionMap.opOrder]),
  body("opId").isInt(),
  body("quantity").isInt(),
    ...
    try {
      const result = await sequelize.transaction(async (t) => {
        ...
        if (quantity > selectedProduct.quantity) {
          ...
          let done = false;
          const anesthesiaItems = opData.anesthesiaItems.map(
            (anesthesiaItem) => {
              if (
                anesthesiaItem.barcode === selectedProduct.barcode &&
                anesthesiaItem.quantity === selectedProduct.quantity &&
                !done
              ) {
                done = true;
                return {
          ...
          const anesthesiaItem = opData.anesthesiaItems.find(
            (anesthesiaItem) =>
              anesthesiaItem.barcode === selectedProduct.barcode &&
              anesthesiaItem.quantity === selectedProduct.quantity
          );
          const anesthesiaItems = opData.anesthesiaItems.filter(
            (i) => i !== anesthesiaItem
          );
          await opData.update(
            {
              ...
              anesthesiaItems,
              totalPriceAnesthesia: sequelize.literal(
                `"totalPriceAnesthesia" - ${totalRefundedPrice}`
              ),
            },
            { transaction: t }
          );
      ...
      const result = await sequelize.transaction(async (t) => {
        ...
        if (quantity > selectedProduct.quantity) {
          ...
          const perfusionItems = opData.perfusionItems.map((perfusionItem) => {
            if (
              perfusionItem.barcode === selectedProduct.barcode &&
              perfusionItem.quantity === selectedProduct.quantity &&
              !done
            ) {
              done = true;
              return {
          ...
          const item = opData.perfusionItems.find(
            (perfusionItem) =>
              perfusionItem.barcode === selectedProduct.barcode &&
              perfusionItem.quantity === selectedProduct.quantity
          );
          const perfusionItems = opData.perfusionItems.filter(
            (i) => i !== item
          );
          await opData.update(
            {
              ...
              totalPricePerfusion: sequelize.literal(
                `"totalPricePerfusion" - ${totalRefundedPrice}`
              ),
              perfusionItems,
            },
            { transaction: t }
    ...
    try {
      const result = await sequelize.transaction(async (t) => {
        ...
        if (quantity > selectedProduct.quantity) {
          ...
          let done = false;
          const scrubNurseItems = opData.scrubNurseItems.map(
            (scrubNurseItem) => {
              if (
                scrubNurseItem.barcode === selectedProduct.barcode &&
                scrubNurseItem.quantity === selectedProduct.quantity &&
                !done
              ) {
                done = true;
                return {
          ...
          await opData.update(
            {
              ...
              totalPriceScrubNurse: sequelize.literal(
                `"totalPriceScrubNurse" - ${totalRefundedPrice}`
              ),
              scrubNurseItems,
            },
            { transaction: t }
          ...
          const item = opData.scrubNurseItems.find(
            (scrubNurseItem) =>
              scrubNurseItem.barcode === selectedProduct.barcode &&
              scrubNurseItem.quantity === selectedProduct.quantity
          );
          const scrubNurseItems = opData.scrubNurseItems.filter(
            (i) => item !== i
          );
          await opData.update(
            {
  ...
  async (req, res) => {
    ...
    try {
      const result = await sequelize.transaction(async (t) => {
        ...
        const totalPrice = opDatas.reduce(
          (acc, current) => acc + current.totalPrice,
          0
        );
        const totalPriceScrubNurse = opDatas.reduce(
          (acc, current) => acc + current.totalPriceScrubNurse,
          0
        );
        const totalPriceAnesthesia = opDatas.reduce(
          (acc, current) => acc + current.totalPriceAnesthesia,
          0
        );
        const totalPricePerfusion = opDatas.reduce(
          (acc, current) => acc + current.totalPricePerfusion,
          0
        );
        const perfusionItems = opDatas.reduce(
          (acc, current) => [...acc, ...current.perfusionItems],
          []
        );
        const anesthesiaItems = opDatas.reduce(
          (acc, current) => [...acc, ...current.anesthesiaItems],
          []
        );
        const scrubNurseItems = opDatas.reduce(
          (acc, current) => [...acc, ...current.scrubNurseItems],
          []
        );
        ...
        const uniqueOperationTypes = new Set(
          opDatas
            .map((op) =>
              op.OPOperationTypes.map(
                (opOperationType) => opOperationType.opOperationTypeId
              )
            )
            .flat()
        );
        await OPDataOperationTypeJunction.bulkCreate(
